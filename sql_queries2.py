...
query5="SELECT emp_NAME,WORK_AREA,DEPT_NAME FROM employee WHERE WORK_AREA IN (SELECT WORK_AREA FROM employee GROUP BY WORK_AREA HAVING COUNT(*) > 1)"
# query5="SELECT* FROM employee WHERE WORK_AREA IN(SELECT WORK_AREA FROM (SELECT WORK_AREA, ROW_NUMBER() OVER (PARTITION BY DEPT_NAME ORDER BY DEPT_NAME) AS row_num FROM employee)as temp_table WHERE row_num>1)"
my_cursor.execute(query5)
for data in my_cursor:
    print(data)
    
#same work area irrespective of their departments.
qu="SELECT e1.emp_NAME,e1.WORK_AREA,e1.DEPT_NAME from employee e1 join employee e2 on e1.WORK_AREA = e2.WORK_AREA and e1.emp_ID <> e2.emp_Id"
